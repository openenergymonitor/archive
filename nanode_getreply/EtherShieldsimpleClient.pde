...
void browserresult_callback(uint8_t statuscode,uint16_t datapos) 
{
  if (datapos != 0)
  {
    ...
    while (buf[pos] && done ==0)
    {
      if (buf[pos]=='\n') done=1;
      if (space_count==1 && !done)
      {
        status_code[si] = buf[pos];
        si++;
      }
      if (buf[pos]== ' ') space_count++;
      pos++;
    }
    status_code[si] = '\0';
    ...
    if (status_code_int== 200) Serial.println("Yes it worked, great!! status 200...");
    if (status_code_int== 404) Serial.println("Hmmm I cant find it.");
