  ...
  function get_input_processlist_desc($id)
  {
    ...
    if ($array['processList']){		
      ...
      foreach ($array as $row)    			// For all input processes
      {
        ...
        if ($processid==1) {$processDescription = "x: ";  $argA = get_feed_name($argA);}
        if ($processid==2) $processDescription = "+: ";
        if ($processid==3) $processDescription = "x input";
        if ($processid==4) {$processDescription = "Log to feed: ";  $argA = get_feed_name($argA);}
        if ($processid==5) {$processDescription = "Power to kWh: ";  $argA = get_feed_name($argA);}
        if ($processid==6) {$processDescription = "Power to kWh/d ";  $argA = get_input_name($argA);}
        if ($processid==7) {$processDescription = "kWhinc to kWh/d: ";  $argA = get_input_name($argA);}
        if ($processid==8) {$processDescription = "input on-time: ";  $argA = get_feed_name($argA);}
